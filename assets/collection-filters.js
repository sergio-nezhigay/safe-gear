class CollectionFilters {
  constructor() {
    ...
    this.currentPage = 1;
    this.itemsPerPage = parseInt(document.querySelector('[data-collections-per-page]')?.getAttribute('data-collections-per-page')) || 12;
    this.showPagination = document.querySelector('[data-show-pagination]')?.getAttribute('data-show-pagination') === 'true';
    this.productsCurrentPage = 1;
    this.productsItemsPerPage = parseInt(document.getElementById('products-grid')?.getAttribute('data-products-per-page')) || 16;
    this.productsServerPagination = document.getElementById('products-grid')?.getAttribute('data-server-pagination') === 'true';
  ...
  cacheDOM() {
    ...
    this.productsPagination = document.getElementById('products-pagination-container');
    ...
    this.showMoreCategoriesBtn = document.querySelector('.show-more-categories');
  ...
  initCurrency() {
    ...
    for (const priceElement of productPrices) {
      if (priceElement && priceElement.textContent.trim()) {
        ...
        if (extractedSymbol && extractedSymbol.length > 0) {
          ...
          foundCurrency = true;
          console.log('Extracted currency from price:', priceText, '→ Symbol:', this.currencySymbol, 'Position:', this.currencyPosition);
          break;
        }
      }
    }
    ...
    if (!foundCurrency && window.L2FilterConfig?.currency) {
      ...
      const currencyMap = {
        ...
        'CHF': 'CHF', 'SEK': 'kr', 'DKK': 'kr', 'NOK': 'kr', 'PLN': 'zł', 'CZK': 'Kč',
        'HUF': 'Ft', 'RUB': '₽', 'CNY': '¥', 'INR': '₹', 'BRL': 'R$', 'MXN': '$',
        'ZAR': 'R', 'THB': '฿', 'SGD': 'S$', 'HKD': 'HK$', 'KRW': '₩'
      };
      
      this.currencySymbol = currencyMap[currency] || currency;
      this.currencyPosition = 'before'; // Default position
      
      console.log('Using fallback currency:', currency, '→ Symbol:', this.currencySymbol);
    }
    ...
    if (!this.currencySymbol || this.currencySymbol === '') {
      ...
      console.log('Using final fallback: $ (dollar)');
    }
  }
  ...
  bindEvents() {
    ...
    this.showMoreCategoriesBtn?.addEventListener('click', () => this.toggleCategoriesVisibility());
    document.querySelectorAll('.filter-group-title[data-accordion]').forEach(title => {
  ...
  updateAvailableSecurityFilters(currentFilterType, dependentFilterType) {
    ...
    this.allCollections.forEach(collection => {
      ...
      if (currentFilters.length === 0 || currentFilters.includes(collection.data[currentFilterType])) {
  ...
  matchesFilters(data) {
    ...
    if (this.filters.price_range.min !== null && data.price < this.filters.price_range.min) {
      return false;
    }
    if (this.filters.price_range.max !== null && data.price > this.filters.price_range.max) {
      return false;
    }
  ...
  renderProducts() {
    ...
    const startIndex = (this.productsCurrentPage - 1) * this.productsItemsPerPage;
    const endIndex = startIndex + this.productsItemsPerPage;
    const pageProducts = new Set(this.filteredProducts.slice(startIndex, endIndex).map(p => p.element));

    this.allProducts.forEach(product => {
      const shouldShow = this.filteredProducts.includes(product) && pageProducts.has(product.element);
      product.element.style.display = shouldShow ? 'flex' : 'none';
    });
  }
  ...
  renderCollections() {
    ...
    const pageCollectionElements = new Set(pageCollections.map(c => c.element));
  ...
  toggleNoResults(show) {
    ...
    if (show && !noResultsElement) {
      ...
      noResultsElement.innerHTML = '<p>No collections match your current filters.</p>';
  ...
  updateResultsCount() {
    ...
    if (countElement) {
      countElement.textContent = `Showing ${count} collection${count !== 1 ? 's' : ''}`;
    }
  }
  ...
  updateProductsPagination() {
    ...
    this.productsPagination.innerHTML = this.generateProductsPaginationHTML(totalPages);

    this.productsPagination.querySelectorAll('.pagination-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.preventDefault();
        const page = parseInt(btn.dataset.page);
  ...
  generateProductsPaginationHTML(totalPages) {
    ...
    if (this.productsCurrentPage > 1) {
      html += `<button class="pagination-btn" data-page="${this.productsCurrentPage - 1}">‹ Previous</button>`;
    }
    ...
    if (this.productsCurrentPage < totalPages) {
      html += `<button class="pagination-btn" data-page="${this.productsCurrentPage + 1}">Next ›</button>`;
    }
  ...
  toggleCategoriesVisibility() {
    ...
    const isExpanded = this.showMoreCategoriesBtn.classList.contains('expanded');
    ...
    if (isExpanded) {
      ...
      const showLessText = document.querySelector('[data-show-less-text]')?.getAttribute('data-show-less-text') || 'Show Less Categories';
