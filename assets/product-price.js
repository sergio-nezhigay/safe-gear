...
import { VariantUpdateEvent } from './events';
...
class ProductPrice extends HTMLElement {
  ...
  updatePriceFromVariant(variant) {
    ...
    if (variant.compare_at_price && variant.compare_at_price > variant.price) {
      const compareElements = priceContainer.querySelectorAll(
        '.price-item--regular s, .price__sale s'
      );
    ...
    if (variant.unit_price_measurement) {
      const unitPriceElement = priceContainer.querySelector(
        '.unit-price .price-item--last span'
      );
