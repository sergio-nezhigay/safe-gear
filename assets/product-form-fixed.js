...
// `VariantUpdateEvent` carries a null resource or when the `available` flag on
// the resource loosely equals `false`.  In Horizon, the `available` property
// may be omitted from the variant data or left as `null`, causing the button
// to remain disabled even when the variant can be purchased.  This version
// tightens the check to only disable when `available` is explicitly `false`.
...
class ProductFormComponent extends Component {
  ...
  /**
   * When the selected variant has changed, update the form.
   *
   * This patched version only disables the add‑to‑cart button when the
   * variant's `available` property is explicitly `false`.  Missing or
   * undefined values will leave the button enabled, allowing purchase.
   *
   * @param {VariantUpdateEvent} event
   */
  #onVariantUpdate = (event) => {
    // If the update corresponds to another product, ignore
    ...
    if (resource) {
      ...
      // Also check inventory if managed by Shopify
      if (resource.inventory_management === 'shopify' && 
          resource.inventory_policy === 'deny' && 
          resource.inventory_quantity <= 0) {
        isAvailable = false;
      }
    } else {
      // No variant resource means no variant is selected
      isAvailable = false;
    }
    ...
    if (!isAvailable) {
      ...
      // Update button text for out of stock
      const buttonText = currentAddToCartButton.querySelector('.add-to-cart-text__content');
      if (buttonText) {
        buttonText.textContent = Theme.translations.sold_out || 'Sold out';
      }
    } else {
      addToCartButtonContainer.enable();
      this.refs.acceleratedCheckoutButtonContainer?.removeAttribute('hidden');
      
      // Restore button text for available
      const buttonText = currentAddToCartButton.querySelector('.add-to-cart-text__content');
      if (buttonText) {
        buttonText.textContent = Theme.translations.add_to_cart || 'Add to cart';
      }
    }
    
    // Swap the button text/icon if a new markup is provided
    if (newAddToCartButton) {
      morph(currentAddToCartButton, newAddToCartButton);
    }
    // Update the hidden input with the variant ID (or clear if missing)
    variantId.value = resource?.id ?? '';
    // If there is media for this variant, attach its URL to the button for fly‑to‑cart
    if (resource) {
...
class FlyToCart extends HTMLElement {
  ...
  #animate() {
    ...
    //Animation variables
    /** @type {number | null} */ let startTime = null;
